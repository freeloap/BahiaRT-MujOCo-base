...
class DecisionMaker:
    ...
    FORMATION_SHIFT_MAX: float = 2.0  # 阵型纵向平移的最大幅度（米）
    ARRIVE_TOL: float = 0.4       # 外场球员到位判定阈值（米）
    GK_ARRIVE_TOL: float = 0.3    # 守门员到位判定阈值（米）
    ORIENT_NEAR: float = 2.0      # 距目标小于此值时才锁定朝向（米）
    GK_LINE_OFFSET: float = 0.6   # 守门员站在己方端线前方的距离（米）
    GK_HALF_WIDTH: float = 1.2    # 守门员横向跟球的半幅（米）
    ...
    # ----------------------------- 阵型站位（7 人 / 9×14 场地）-----------------------------
    # 己方球门在 -x，进攻朝 +x，y∈[-4.5,4.5]，开球时全员位于己方半场（x<0）。
    BEAM_POSES: Mapping[type[Field], Mapping[int, tuple[float, float, float]]] = {
        ...
        # 比赛场地（9×14，7v7）。号码 1 必须是守门员（规则要求）。
        HLAdultField: {
            1: (-6.3, 0.0, 0),   # 守门员
            2: (-4.5, 1.6, 0),   # 左后卫
            3: (-4.5, -1.6, 0),  # 右后卫
